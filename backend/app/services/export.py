...
def generate_pdf_statement_bytes(
    ...
    header_data = [
        [
            Paragraph("FISCALPREMIUM CHARTERED ACCOUNTING", title_style),
    ...
    elements.append(HRFlowable(width="100%", thickness=2, color=colors.HexColor("#10B981"), spaceAfter=15, spaceBefore=5))
    ...
    summary_data = [
        ...
        [
            ...
            Paragraph(f"<b>{pnl.profit_margin_percentage}</b>", cell_style_right)
        ]
    ]
    ...
    elements.append(Paragraph("This document is dynamically generated by FiscalPremium AI Accounting Assistant (Powered by Gemini 2.0 / PydanticAI Engine). All calculations are deterministic and verified against double-entry database rules.", footer_style))
...
def generate_csv_statement_string(
    ...
    # Metadata headers
    writer.writerow(["# FISCALPREMIUM AI ACCOUNTING ASSISTANT - FINANCIAL STATEMENT"])
    ...
    writer.writerow(["Profit Margin (%)", pnl.profit_margin_percentage])
    writer.writerow([])
